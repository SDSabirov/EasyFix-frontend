<template>
   ...
   >
     ...
     <form @submit.prevent="submitForm" class="mb-6">
       ...
       <button
         ...
         :class="[isSubmitted ? 'bg-green-500' : 'bg-primary', 'text-white w-full focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 dark:focus:ring-blue-800 block']"
       >
 ...
 const submitForm = async () => {
   ...
   try {
     ...
     const response = await fetch('https://api.easyfixappliance.com/api/contact-form', {
      method: 'POST',
      headers: {
